...
fetchPopularSeriesData().then((series) => {
  series.forEach((serie) => {
  ...
  function searchMovie(movie_name) {
    ...
    return fetch(
      `https://api.themoviedb.org/3/search/movie?query=${modifiedMovieName}&include_adult=false&language=en-US&page=1`,
      options
    )
      .then((response) => response.json())
      .then((data) => data.results)
  ...
  async function aiRequest() {
    ...
    const length =
      document.querySelector(
        ".button-group button.selected[data-type='length']"
      )?.innerText || "medium";
  ...
  async function displayRecommendations(recommendations) {
    ...
    for (const movieName in recommendations) {
      if (recommendations.hasOwnProperty(movieName)) {
        ...
        try {
          const movieData = await searchMovie(movieName).then(
            (data) => data[0]
          );
          if (movieData) {
